...
# cargo update shiguredo_vmaf を実行
def run_cargo_update(dry_run: bool) -> None:
    if dry_run:
        print("Dry-run: Would run 'cargo update shiguredo_vmaf'")
    else:
        subprocess.run(["cargo", "update", "shiguredo_vmaf"], check=True)
        print("cargo update shiguredo_vmaf executed")
